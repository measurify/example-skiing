...
class ChartDataIMU {
  ...
  ChartDataIMU(this.timestamp, this.accX,this.accY,this.accZ,this.gyrX,this.gyrY,this.gyrZ,this.magX,this.magY,this.magZ);
}
...
Future<List<String>> getAllMeasurements() async {
    ...
    var request = http.Request(
        'GET',
        Uri.parse(
            widget.globals.url+'measurements?select["_id"]&limit=-1'));
    ...
    if (response.statusCode == 200) {
      ...
      allMeasurements.addAll(
          measurements["docs"].map((measure) => measure["_id"].toString()).cast<String>());
    } else {
...
Future <List<dynamic>> getSelectTimeseries() async {
    ...
    var request = http.Request(
        'GET',
        Uri.parse(
            widget.globals.url+'measurements/'+ _selectedOption +'/timeserie?limit=300&sort={"timestamp":"asc"}'));
...
return {       "timestamp": timeserie["timestamp"],     
...
"magZ": timeserie["values"][8].toDouble(),    };  
  ...
  children: [
      ...
      onChanged: (String? newValue) async {
        ...
        setState(() {
          ...
          chartDataIMUList.addAll(measurements.map((measure) {
            return ChartDataIMU(measure["timestamp"], measure["accX"], measure["accY"], measure["accZ"], measure["gyrX"], measure["gyrY"], measure["gyrZ"], measure["magX"], measure["magY"], measure["magZ"]);
          }));
        });
      },
      items: allMeasurements.map<DropdownMenuItem<String>>((String value) {
    ...
    Expanded(
      child: Row(
        children: <Widget>[
          Expanded( // Wrap the chart call with Expanded
  ...
  SfCartesianChart _buildDefaultLineChart() {
    return SfCartesianChart(
      ...
      primaryXAxis: CategoryAxis(interval: 2, labelRotation: 80),/*const NumericAxis(
